...
export const createHeadersParser: TCreateHeadersParser = (headers) => async (ctx, next) => {
  ...
  const { error, value } = Joi.object().keys(schema).validate(headerMap, { abortEarly: false })
